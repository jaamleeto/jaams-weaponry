...
@OnlyIn(Dist.CLIENT)
public class RingProjectileRenderer extends EntityRenderer<RingProjectileEntity> {
    public RingProjectileRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public void render(RingProjectileEntity entity, float entityYaw, float partialTicks, PoseStack matrixStack,
        ...
        float scale = data.getFloat("ProjectileScale", (render != null && render.scale != null) ? render.scale
                : ProjectileClientConfig.RING_PROJECTILE_SCALE.get().floatValue());
