...
@OnlyIn(Dist.CLIENT)
public class SharpStoneProjectileRenderer extends EntityRenderer<SharpStoneProjectileEntity> {
    public SharpStoneProjectileRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public void render(SharpStoneProjectileEntity entity, float entityYaw, float partialTicks, PoseStack matrixStack,
        ...
        float scale = data.getFloat("ProjectileScale", (render != null && render.scale != null) ? render.scale
                : ProjectileClientConfig.SHARP_STONE_PROJECTILE_SCALE.get().floatValue());
        ...
        float offsetY = data.getFloat("ProjectileOffsetY",
                (render != null && render.offset_y != null) ? render.offset_y : -0.075f);
    ...
    @Override
    public ResourceLocation getTextureLocation(SharpStoneProjectileEntity entity) {
