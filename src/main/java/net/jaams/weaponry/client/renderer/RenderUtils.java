...
public class RenderUtils {
    ...
    public static void renderCustomSun(RenderLevelStageEvent event, ResourceLocation texture) {
        ...
        float f11 = 1.0F - mc.getInstance().player.level().getRainLevel(event.getPartialTick().getGameTimeDeltaPartialTick(false));
        RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, f11);
        posestack.mulPose(Axis.YP.rotationDegrees(-90.0F));
        posestack.mulPose(
                Axis.XP.rotationDegrees(mc.getInstance().player.level().getTimeOfDay(event.getPartialTick().getGameTimeDeltaPartialTick(false)) * 360.0F));
