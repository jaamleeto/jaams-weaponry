...
@OnlyIn(Dist.CLIENT)
public class CleaverProjectileRenderer extends EntityRenderer<CleaverProjectileEntity> {
    public CleaverProjectileRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public void render(CleaverProjectileEntity entity, float entityYaw, float partialTicks, PoseStack matrixStack,
        ...
        float scale = data.getFloat("ProjectileScale", (render != null && render.scale != null) ? render.scale
                : ProjectileClientConfig.CLEAVER_PROJECTILE_SCALE.get().floatValue());
        ...
        float offsetY = data.getFloat("ProjectileOffsetY",
                (render != null && render.offset_y != null) ? render.offset_y : -0.175f);
    ...
    @Override
    public ResourceLocation getTextureLocation(CleaverProjectileEntity entity) {
