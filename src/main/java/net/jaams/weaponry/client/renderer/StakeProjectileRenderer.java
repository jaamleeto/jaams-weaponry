...
@OnlyIn(Dist.CLIENT)
public class StakeProjectileRenderer extends EntityRenderer<StakeProjectileEntity> {
    public StakeProjectileRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public void render(StakeProjectileEntity entity, float entityYaw, float partialTicks, PoseStack matrixStack,
        ...
        float scale = data.getFloat("ProjectileScale", (render != null && render.scale != null) ? render.scale
                : ProjectileClientConfig.STAKE_PROJECTILE_SCALE.get().floatValue());
        ...
        float offsetY = data.getFloat("ProjectileOffsetY",
                (render != null && render.offset_y != null) ? render.offset_y : -0.15f);
        ...
        matrixStack.mulPose(Axis.ZP.rotationDegrees(lerpPitch + pitchOffset));
    ...
    @Override
    public ResourceLocation getTextureLocation(StakeProjectileEntity entity) {
