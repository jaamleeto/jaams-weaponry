...
@OnlyIn(Dist.CLIENT)
public class RoyalSpearProjectileRenderer extends EntityRenderer<RoyalSpearProjectileEntity> {
    public RoyalSpearProjectileRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public void render(RoyalSpearProjectileEntity entity, float entityYaw, float partialTicks, PoseStack matrixStack,
        ...
        float scale = data.getFloat("ProjectileScale", (render != null && render.scale != null) ? render.scale
                : ProjectileClientConfig.ROYAL_SPEAR_PROJECTILE_SCALE.get().floatValue());
        float rotX = data.getFloat("ProjectileRotX", -90.0f);
        float rotY = data.getFloat("ProjectileRotY",
                (render != null && render.yaw_offset != null) ? render.yaw_offset : -180.0f);
        ...
        float offsetY = data.getFloat("ProjectileOffsetY",
                (render != null && render.offset_y != null) ? render.offset_y : -1.195f);
        ...
        matrixStack.mulPose(Axis.XP.rotationDegrees(lerpPitch + rotX + pitchOffset));
    ...
    @Override
    public ResourceLocation getTextureLocation(RoyalSpearProjectileEntity entity) {
