...
@OnlyIn(Dist.CLIENT)
public class BroomProjectileRenderer extends EntityRenderer<BroomProjectileEntity> {
    public BroomProjectileRenderer(EntityRendererProvider.Context context) {
        super(context);
    }

    @Override
    public void render(BroomProjectileEntity entity, float entityYaw, float partialTicks, PoseStack matrixStack,
        ...
        float scale = data.getFloat("ProjectileScale", (render != null && render.scale != null) ? render.scale
                : ProjectileClientConfig.BROOM_PROJECTILE_SCALE.get().floatValue());
        ...
        float offsetY = data.getFloat("ProjectileOffsetY",
                (render != null && render.offset_y != null) ? render.offset_y : -0.895f);
    ...
    @Override
    public ResourceLocation getTextureLocation(BroomProjectileEntity entity) {
