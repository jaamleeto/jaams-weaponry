...
public class HeavyBulletProjectileEntity extends BaseBulletProjectileEntity {
    ...
    public HeavyBulletProjectileEntity(PlayMessages.SpawnEntity packet, Level world) {
        super(ModEntities.HEAVY_BULLET_PROJECTILE.get(), world);
    ...
    protected void applyHitEffects(Entity entity, EntityHitResult hitResult) {
        ...
        if (knockedOutChance > 0 && level().random.nextDouble() < knockedOutChance
            ...
            if (duration > 0) {
                living.addEffect(
                        new MobEffectInstance(ModMobEffects.KNOCKED_OUT.get(), duration, 0, false, false, true));
            }
        }
