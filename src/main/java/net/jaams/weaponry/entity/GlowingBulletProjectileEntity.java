...
public class GlowingBulletProjectileEntity extends BaseBulletProjectileEntity {

    public GlowingBulletProjectileEntity(PlayMessages.SpawnEntity packet, Level world) {
        super(ModEntities.GLOWING_BULLET_PROJECTILE.get(), world);
        initializeProperties();
    }

    public GlowingBulletProjectileEntity(EntityType<? extends GlowingBulletProjectileEntity> type, Level world) {
        super(type, world);
        initializeProperties();
    }

    public GlowingBulletProjectileEntity(EntityType<? extends GlowingBulletProjectileEntity> type, double x, double y,
            double z, Level world) {
        super(type, x, y, z, world);
        initializeProperties();
    }

    public GlowingBulletProjectileEntity(EntityType<? extends GlowingBulletProjectileEntity> type, LivingEntity entity,
            Level world) {
        super(type, entity, world);
        initializeProperties();
    }

    public GlowingBulletProjectileEntity(Level world, LivingEntity shooter, ItemStack gunItem) {
        super(ModEntities.GLOWING_BULLET_PROJECTILE.get(), shooter, world, gunItem);
        initializeProperties();
    }
    ...
    private void initializeProperties() {
        ...
        this.setBulletDamage(
                ModProjectiles.getBaseDamage(gun, ProjectileBulletConfig.GLOWING_BULLET_PROJECTILE_BASE_DAMAGE.get()));
        this.setBulletKnockback(ModProjectiles.getBaseKnockback(gun,
                ProjectileBulletConfig.GLOWING_BULLET_PROJECTILE_BASE_KNOCKBACK.get()));
        this.setPiercingLevel(ModProjectiles.getPiercingLevel(gun,
                ProjectileBulletConfig.GLOWING_BULLET_PROJECTILE_PIERCING_LEVEL.get()));
        this.setColor(ModProjectiles.getColor(gun, ProjectileBulletConfig.GLOWING_BULLET_PROJECTILE_COLOR));
    }
    ...
    protected void applyHitEffects(Entity entity, EntityHitResult hitResult) {
        boolean applyGlowing = ModProjectiles.getGlowing(this.getGunItem(),
                ProjectileBulletConfig.GLOWING_BULLET_PROJECTILE_GLOWING.get());
        int glowDuration = ModProjectiles.getGlowingDuration(this.getGunItem(),
                ProjectileBulletConfig.GLOWING_BULLET_PROJECTILE_GLOW_DURATION.get());
        if (applyGlowing && glowDuration > 0 && entity instanceof LivingEntity living) {
            living.addEffect(new MobEffectInstance(MobEffects.GLOWING, glowDuration, 0, false, false, true));
        }
        double magicDamageChance = ModProjectiles.getMagicDamageChance(this.getGunItem(),
                ProjectileBulletConfig.GLOWING_BULLET_PROJECTILE_MAGIC_DAMAGE_CHANCE.get());
        if (magicDamageChance > 0 && level().random.nextDouble() < magicDamageChance) {
            double magicDamage = ModProjectiles.getMagicDamage(this.getGunItem(),
                    ProjectileBulletConfig.GLOWING_BULLET_PROJECTILE_MAGIC_DAMAGE.get());
            if (magicDamage > 0 && entity instanceof LivingEntity living) {
                living.hurt(new DamageSource(level().registryAccess().registryOrThrow(Registries.DAMAGE_TYPE)
                        .getHolderOrThrow(DamageTypes.MAGIC), this, this), (float) magicDamage);
            }
        }
        boolean showParticles = ModProjectiles.getShowAmethystParticles(this.getGunItem(),
                ProjectileBulletConfig.GLOWING_BULLET_PROJECTILE_SHOW_AMETHYST_PARTICLES.get());
        if (showParticles && level() instanceof ServerLevel serverLevel) {
            ItemStack particleStack = new ItemStack(Items.AMETHYST_SHARD);
    ...
    protected void applyBlockHitEffects(BlockPos pos, BlockState state, BlockHitResult hitResult) {
        ...
        if (showParticles && level() instanceof ServerLevel serverLevel) {
            ...
            ItemStack itemStack = new ItemStack(Items.AMETHYST_SHARD);
    ...
    public boolean canDisableShield(ItemStack shield, LivingEntity shieldHolder, Entity owner) {
        return ModProjectiles.canDisableShield(this.getGunItem(),
                ProjectileBulletConfig.GLOWING_BULLET_PROJECTILE_DISABLE_SHIELD.get());
    }
    ...
    public int getIgnoreHitTicks() {
        return ModProjectiles.getIgnoreHitTicks(this.getGunItem(),
                ProjectileBulletConfig.GLOWING_BULLET_PROJECTILE_IGNORE_TICKS.get());
    }
    ...
    public boolean isCritical() {
        if (!ModProjectiles.getAllowCriticals(this.getGunItem(),
                ProjectileBulletConfig.GLOWING_BULLET_PROJECTILE_ALLOW_CRITICALS.get())) {
            return false;
        }
