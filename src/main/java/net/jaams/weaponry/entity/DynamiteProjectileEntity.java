...
public class DynamiteProjectileEntity extends BaseWeaponProjectileEntity {
    ...
    protected void defineSynchedData(SynchedEntityData.Builder builder) {
        ...
        builder.define(ID_TICKS_ON_GROUND, 0);
    }
    ...
    protected SoundEvent getHitSound() {
        return ModProjectiles.getCustomProjectileSound(this.getWeaponItem(), "ProjectileHitSound", "projectile_hit",
                "empty_sound", SoundEvents.GENERIC_EXPLODE.value(), (projectileEntry) -> projectileEntry.hit_sound);
    }
    ...
    protected SoundEvent getGroundSound() {
        return ModProjectiles.getCustomProjectileSound(this.getWeaponItem(), "ProjectileGroundSound",
                "projectile_ground", "empty_sound", SoundEvents.GENERIC_EXPLODE.value(),
    ...
    protected void tickDespawn() {
        ...
        if (life >= despawnTime) {
            if (ProjectileCommonConfig.DYNAMITE_PROJECTILE_DESPAWN_AS_ITEM.get()
                    && this.pickup != AbstractArrow.Pickup.CREATIVE_ONLY && !isIgnited()) {
    ...
    protected SoundEvent getIgnitingSound() {
        ...
        if (weapon != null && ModComponents.has(weapon)
                && ModComponents.get(weapon).contains(NBT_IGNITING_SOUND, net.minecraft.nbt.Tag.TAG_STRING)) {
            String soundId = ModComponents.get(weapon).getString(NBT_IGNITING_SOUND);
            SoundEvent custom = BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse(soundId));
            if (custom != null) {
                return custom;
            }
        }
        return BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("jaams_weaponry:dynamite_igniting"));
    }
    ...
    private void spawnItemEntity() {
        if (!this.level().isClientSide && !isIgnited()) {
