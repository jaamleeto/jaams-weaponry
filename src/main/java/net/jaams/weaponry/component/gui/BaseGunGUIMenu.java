...
public abstract class BaseGunGUIMenu extends AbstractContainerMenu implements Supplier<Map<Integer, Slot>> {
	...
	protected void slotChanged(int slotid) {
		if (world.isClientSide()) {
			...
			if (!ItemStack.matches(current, lastKnownStacks.getOrDefault(slotid, ItemStack.EMPTY))) {
	...
	protected void playClientSound(ResourceLocation sound) {
		...
		if (soundEvent == null)
			return;
		world.playLocalSound(x + 0.5, y + 0.5, z + 0.5, soundEvent, SoundSource.PLAYERS, 1.0F, 1.0F, false);
	}

	private void playSlotSound(int slotid) {
		if (!GunSystemClientConfig.GUN_INV_SOUNDS.get() || !world.isClientSide())
			return;
		ResourceLocation sound = getSlotSound(slotid);
		if (sound == null)
			return;
		SoundEvent soundEvent = ForgeRegistries.SOUND_EVENTS.getValue(sound);
		if (soundEvent == null)
		...
		world.playLocalSound(x + 0.5, y + 0.5, z + 0.5, soundEvent, SoundSource.PLAYERS, 1.0F, pitch, false);
	}
