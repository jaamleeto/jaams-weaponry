...
public abstract class BaseItemProjectileEntity extends AbstractArrow implements IEntityWithComplexSpawn {
    ...
    protected void defineSynchedData(SynchedEntityData.Builder builder) {
        ...
        builder.define(ID_SOURCE_ITEM, ItemStack.EMPTY);
        builder.define(ID_PROJECTILE_ITEM, ItemStack.EMPTY);
        builder.define(ID_CRITICAL, false);
        builder.define(ID_IGNORE_HIT_TICKS, 0);
        builder.define(ID_PIERCING, 0);
        builder.define(ID_COLOR, 0xFFFFFF);
    }
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        if (tag.contains("SourceItem", 10)) {
            this.sourceItem = ItemStack.parseOptional(this.registryAccess(), tag.getCompound("SourceItem"));
        } else {
            this.sourceItem = getDefaultSourceItem();
        }
        this.entityData.set(ID_SOURCE_ITEM, this.sourceItem);
        if (tag.contains("ProjectileItem", 10)) {
            this.projectileItem = ItemStack.parseOptional(this.registryAccess(), tag.getCompound("ProjectileItem"));
        } else {
            this.projectileItem = getDefaultProjectileItem();
        }
        this.entityData.set(ID_PROJECTILE_ITEM, this.projectileItem);
        this.getPersistentData().put("SourceItem", this.sourceItem.saveOptional(this.registryAccess()));
        this.getPersistentData().put("ProjectileItem", this.projectileItem.saveOptional(this.registryAccess()));
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        if (!this.sourceItem.isEmpty()) {
            tag.put("SourceItem", this.sourceItem.saveOptional(this.registryAccess()));
        }
        if (!this.projectileItem.isEmpty()) {
            tag.put("ProjectileItem", this.projectileItem.saveOptional(this.registryAccess()));
        }
    ...
    public void writeSpawnData(RegistryFriendlyByteBuf buffer) {
        ItemStack.OPTIONAL_STREAM_CODEC.encode(buffer, this.sourceItem);
        ItemStack.OPTIONAL_STREAM_CODEC.encode(buffer, this.projectileItem);
    ...
    public void readSpawnData(RegistryFriendlyByteBuf buf) {
        this.sourceItem = ItemStack.OPTIONAL_STREAM_CODEC.decode(buf);
        this.projectileItem = ItemStack.OPTIONAL_STREAM_CODEC.decode(buf);
        if (this.sourceItem.isEmpty())
            this.sourceItem = getDefaultSourceItem();
        if (this.projectileItem.isEmpty())
            this.projectileItem = getDefaultProjectileItem();
    ...
    protected void syncPersistentDataToEntityData() {
        ...
        if (persistent.contains("SourceItem", 10)) {
            ItemStack persistentSource = ItemStack.parseOptional(this.registryAccess(), persistent.getCompound("SourceItem"));
        ...
        if (persistent.contains("ProjectileItem", 10)) {
            ItemStack persistentProjectile = ItemStack.parseOptional(this.registryAccess(), persistent.getCompound("ProjectileItem"));
    ...
    protected void applyKnockback(LivingEntity target) {
        ...
        int knockbackLevel = ModEnchantments.level(this.projectileItem, Enchantments.KNOCKBACK);
        if (knockbackLevel > 0) {
    ...
    protected float calculateProjectileDamage(ItemStack projectileItem, float baseWeaponDamage,
        ...
        if (hitEntity instanceof LivingEntity livingEntity) {
            baseDamage += ModEnchantments.damageBonus(projectileItem, livingEntity);
            int powerLevel = ModEnchantments.level(projectileItem, Enchantments.POWER);
    ...
    public void updateEnchantmentData() {
        if (this.projectileItem.isEmpty())
            return;
        this.entityData.set(ID_LOYALTY, (byte) ModEnchantments.level(this.projectileItem, net.minecraft.world.item.enchantment.Enchantments.LOYALTY));
        this.entityData.set(ID_FOIL, this.projectileItem.hasFoil());
        int enchantPierce = ModEnchantments.level(this.projectileItem, Enchantments.PIERCING);
    ...
    protected void applyCompatKnockback(net.minecraft.world.entity.LivingEntity target) {
        if (this.knockback <= 0) {
            return;
        }
        ...
        net.minecraft.world.phys.Vec3 push = this.getDeltaMovement().multiply(1.0D, 0.0D, 1.0D).normalize().scale(this.knockback * 0.6D * resistance);
