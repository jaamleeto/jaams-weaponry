...
public class GunItemData {
    ...
    public AttachmentEntry attachment = new AttachmentEntry();
    ...
    public static Optional<AttachmentEntry> getAttachmentData(ItemStack stack) {
        if (stack == null || stack.isEmpty())
            return Optional.empty();
        GunItemData data = getData(stack).orElse(null);
        if (data == null || data.attachment == null || data.attachment.slot == null || data.attachment.slot.isEmpty())
            return Optional.empty();
        return Optional.of(data.attachment);
    }

    public static class AttachmentEntry {
        public String slot = "";
        public List<String> compatible_guns = new ArrayList<>();
        public List<AttachmentModifier> modifiers = new ArrayList<>();
        public String fire_pattern = "";
        public SoundEntry sound = new SoundEntry();
        public ParticleEntry particle = new ParticleEntry();
    }

    public static class AttachmentModifier {
        public String key = "";
        public double value = 0.0;
        public String operation = "multiply";
    }
}
