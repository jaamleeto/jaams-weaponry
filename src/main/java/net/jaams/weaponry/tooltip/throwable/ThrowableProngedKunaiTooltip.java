...
public class ThrowableProngedKunaiTooltip {
    public static void add(ItemStack stack, List<Component> tooltip) {
        ...
        if (ModComponents.has(stack)) {
            for (String key : ModComponents.get(stack).getAllKeys()) {
                if (key.startsWith("Force") && key.endsWith("Throwable")) {
                    if (ModComponents.get(stack).getBoolean(key)) {
                        return;
                    }
                }
            }
    ...
    private static void addPiercingLevel(ItemStack stack, List<Component> tooltip) {
        ...
        piercingLevel += ModEnchantments.level(stack, Enchantments.PIERCING);
        if (piercingLevel > 0) {
