...
public class ThrowableShurikenTooltip {
    ...
    private static boolean isShuriken(ItemStack stack) {
        return stack.is(ModTags.SHURIKENS) || ModUtils.getBooleanNBT(stack, "ForceShurikenThrowable") == Boolean.TRUE;
    }
    ...
    private static boolean isThrowableEnabled(ItemStack stack) {
        ...
        if (!isShuriken(stack))
            return false;
        if (!ThrowableConfig.THROWABLE_SHURIKEN.get())
            return false;
    ...
    private static ModEnums.ThrowMode getThrowMode(ItemStack stack) {
        ...
        return ThrowableConfig.THROWABLE_SHURIKEN_THROW_MODE.get();
    }
    ...
    private static void addMainTraits(ItemStack stack, List<Component> tooltip) {
        ...
        if (isDisablingShotEnabled(stack)) {
    ...
    private static void addThrowingPropertiesIfEnabled(ItemStack stack, List<Component> tooltip,
        ...
        addDisablingShotPropertiesIfEnabled(stack, tooltip);
    }
    ...
    private static void addDisablingShotPropertiesIfEnabled(ItemStack stack, List<Component> tooltip) {
        ...
        int cooldownTicks = ModProjectiles.getDisablingShotCooldown(stack,
    ...
    private static void addBaseDamage(ItemStack stack, List<Component> tooltip) {
        ...
        if (nbtDamage != null) {
            ...
            baseDamage += ProjectileCommonConfig.SHURIKEN_PROJECTILE_BASE_DAMAGE.get();
        }
        baseDamage += 1.0;
    ...
    private static void addChargeTimesIfApplicable(ItemStack stack, List<Component> tooltip, boolean isInstant) {
        ...
        if (minCharge == null || minCharge <= 0) {
            minCharge = ThrowableConfig.THROWABLE_SHURIKEN_MIN_CHARGE.get();
        }
    ...
    private static void addSpeedStats(ItemStack stack, List<Component> tooltip, boolean isInstant) {
        ...
        double minSpeed = (minSpeedNBT != null && minSpeedNBT > 0) ? minSpeedNBT
                : ThrowableConfig.THROWABLE_SHURIKEN_MIN_SPEED.get().doubleValue();
    ...
    private static void addPiercingLevel(ItemStack stack, List<Component> tooltip) {
        int piercingLevel = ProjectileCommonConfig.SHURIKEN_PROJECTILE_PIERCING_LEVEL.get();
    ...
    private static void addCooldownIfInstant(ItemStack stack, List<Component> tooltip, boolean isInstant) {
        ...
        int cooldownTicks = (cooldown != null && cooldown >= 0) ? cooldown
                : ThrowableConfig.THROWABLE_SHURIKEN_COOLDOWN_TICKS_INSTANT.get();
    ...
    private static void addWaterInertia(ItemStack stack, List<Component> tooltip) {
        ...
        double waterInertia = (inertiaNBT != null && inertiaNBT > 0) ? inertiaNBT
                : ProjectileCommonConfig.SHURIKEN_PROJECTILE_WATER_INERTIA.get().doubleValue();
