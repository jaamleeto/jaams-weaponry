...
import net.jaams.weaponry.tooltip.item.PepperboxItemTooltip;
...
public class TooltipHelper {
    ...
    public static void addAllTooltips(ItemStack stack, List<Component> tooltip) {
        ...
        PepperboxItemTooltip.add(stack, tooltip);
        ...
        if (!isDataDrivenThrowable(stack)
                && !ThrowableModifierLoader.INSTANCE.isThrowableExplicitlyDisabled(stack)) {
    ...
    /**
     * Renders the per-chamber / per-slot ammunition lines for guns. Kept in a
     * shared helper so both the generic {@code Item} tooltip mixin and the
     * {@code CrossbowItem} mixin (which overrides {@code appendHoverText} without
     * calling super) can render them.
     */
    public static void addGunInventoryTooltips(ItemStack stack, List<Component> tooltip) {
        ModGuns.GunType gunType = ModGuns.getGunType(stack);
        if (!ModGuns.isGun(stack) && gunType == null)
            return;
        ...
        CapHelper.itemHandler(stack).ifPresent((handler) -> {
            ...
            for (int i = 0; i < slotCount; i++) {
                ...
                boolean chamberedGun = gunType == ModGuns.GunType.REVOLVER
                        || gunType == ModGuns.GunType.PEPPERBOX;
                if (chamberedGun) {
                    if (i >= 1 && i <= 6) {
                        ...
                        String chamberLabel = TooltipsConfig.TOOLTIP_GUN_CHAMBERS_ROMAN_NUMERALS.get()
