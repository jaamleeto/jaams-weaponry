...
public final class EpicFightCompat {
    ...
    /**
     * Whether Epic Fight's own attribute/weapon tooltips are enabled (config
     * {@code ingame.show_epicfight_attributes}). When they are, Epic Fight already adds its
     * custom attribute lines to the tooltip, so this mod's attribute tooltip rewrite must not
     * run to avoid duplicating (and garbling) that information.
     */
    public static boolean isEpicFightTooltipActive() {
        if (!isEpicFightLoaded()) {
            return false;
        }
        try {
            return EpicFightImpl.showAttributesInTooltip();
        } catch (Throwable t) {
            return false;
        }
    }
    ...
    private static final class EpicFightImpl {
        ...
        static boolean showAttributesInTooltip() {
            return yesman.epicfight.config.ClientConfig.showEpicFightAttributesInTooltip;
        }
    ...
    private static void jaams$updateAndApplyHandBlend(Armature armature, Pose pose, Player player,
        ...
        if (!blendEnabled) {
            ...
            // Exponential smoothing with a small dead-zone so tiny numerical differences do not
            // keep the arm trembling near 0 or 1.
    ...
    private static boolean jaams$canShowAimingPose(Player player, ItemStack stack, boolean isMainHand) {
        ...
        if (nbtForce != null) {
            if (!nbtForce)
                return false;
            // NBT forces the aiming pose on. For recognized guns fall through to the normal ammo
            // checks; for any other item just verify it is the held item.
            if (gunType == null) {
            ...
            Boolean dataForce = jaams$parseAimingPoseValue(gunData.gun_pose);
            if (dataForce != null) {
                if (!dataForce)
                    return false;
            } else {
            ...
            // Non-gun items forced into the pose via tag or gun_modifier skip ammo checks.
            if (gunType == null) {
                ItemStack handStack = isMainHand ? player.getMainHandItem() : player.getOffhandItem();
                return ItemStack.matches(stack, handStack);
            }
    ...
    private static void jaams$applyHandGunAimingPose(Armature armature, Pose pose, Player player,
        ...
        // Use the interpolated view pitch so the arm tracks the head exactly like the rendered
        // model. Raw getXRot() on remote (laggy) players jumps in server-tick steps and makes
        // the aim pose point somewhere behind where the head already is.
        ...
        // Convert the base joint rotation to model space so we can preserve its roll while
        // applying the original aim pitch/yaw. This avoids the wrist/forearm spinning around its
        // own axis when the animation's natural roll differs from zero.
        ...
        // Same aim semantics as the original code: fixed yaw offset and pitch that follows the
        // head. The only difference is that we keep the base animation's roll instead of forcing
        // it to zero, which was causing the unwanted twist during the blend.
        float targetLocalPitch = AIM_X_ROT + headXRot;
        float targetLocalYaw = headYRotOffset;
        float targetLocalRoll = -modelEuler.z;

        // buildArmRotation uses rotationXYZ(-x, y, -z), so model x/z are negated relative to the
        // vanilla-local angles.
        float targetModelX = -targetLocalPitch;
        float targetModelY = targetLocalYaw;
        float targetModelZ = -targetLocalRoll; // == modelEuler.z
        ...
        // Blend using SLERP on the shortest arc. JointTransform.interpolate mixes linearly and
        // can spin the long way around when the base and aim poses are far apart.
    ...
    private static Quaternionf jaams$buildArmRotation(Armature armature, String jointName,
        ...
        new Quaternionf(chainInv)
                ...
                .mul(chain, rotation);
        // Keep the result normalized so repeated per-frame multiplications do not drift and slowly
        // bend the arm away from the desired direction.
    ...
    private static int jaams$getFinalAmmoConsumption(ItemStack gunStack) {
        ...
        switch (type) {
            case PISTOL:
                result = GunSystemCommonConfig.GUN_PISTOL_SHOOT_AMMO_CONSUMPTION.get();
                break;
            case SCATTERGUN:
                result = GunSystemCommonConfig.GUN_SCATTERGUN_SHOOT_AMMO_CONSUMPTION.get();
                break;
            case SHOTGUN:
                result = GunSystemCommonConfig.GUN_SHOTGUN_SHOOT_AMMO_CONSUMPTION.get();
                break;
            case REVOLVER:
                result = GunSystemCommonConfig.GUN_REVOLVER_SHOOT_AMMO_CONSUMPTION.get();
                break;
            case PEPPERBOX:
                result = 1;
                break;
