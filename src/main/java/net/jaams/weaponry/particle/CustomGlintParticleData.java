...
public class CustomGlintParticleData implements ParticleOptions {
    ...
    public static final Codec<CustomGlintParticleData> CODEC = RecordCodecBuilder.create((instance) ->
        instance
    ...
    public static final Deserializer<CustomGlintParticleData> DESERIALIZER = new Deserializer<>() {
        @Override
        public CustomGlintParticleData fromCommand(ParticleType<CustomGlintParticleData> type, StringReader reader) throws CommandSyntaxException {
            reader.expect(' ');
            ...
            return new CustomGlintParticleData(r, g, b, size);
        }

        @Override
        public CustomGlintParticleData fromNetwork(ParticleType<CustomGlintParticleData> type, FriendlyByteBuf buffer) {
            return new CustomGlintParticleData(Mth.clamp(buffer.readFloat(), 0.0F, 1.0F), Mth.clamp(buffer.readFloat(), 0.0F, 1.0F), Mth.clamp(buffer.readFloat(), 0.0F, 1.0F), Mth.clamp(buffer.readFloat(), 0.1F, 2.0F));
        }
    };
