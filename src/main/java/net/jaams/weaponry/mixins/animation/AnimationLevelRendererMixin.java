...
public abstract class AnimationLevelRendererMixin {
    ...
    private void fakeThirdPersonMode(PoseStack poseStack, float partialTick, long finishNanoTime,
            boolean renderBlockOutline, Camera camera, GameRenderer gameRenderer, LightTexture lightTexture,
            Matrix4f projectionMatrix, CallbackInfo ci) {
        if (master == null) {
    ...
    private void resetThirdPerson(PoseStack poseStack, float partialTick, long finishNanoTime,
            boolean renderBlockOutline, Camera camera, GameRenderer gameRenderer, LightTexture lightTexture,
            Matrix4f projectionMatrix, CallbackInfo ci) {
        if (master == null) {
            if (!AnimationAPI.animations.isEmpty())
