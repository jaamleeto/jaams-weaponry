...
public abstract class CustomSmokeBombMixin {
    ...
    private boolean shouldApplyCustomSmokeBombLogic() {
        ...
        if (!ModCompats.isSmokeBomb(stack))
            return false;
        if (stack.hasTag() && stack.getTag().contains("SmokeBomb")) {
            return stack.getTag().getBoolean("SmokeBomb");
        }
        return true;
    }
    ...
    private void jaam$modifySmokeBombUseDuration(CallbackInfoReturnable<Integer> cir) {
        if (!shouldApplyCustomSmokeBombLogic())
            return;
