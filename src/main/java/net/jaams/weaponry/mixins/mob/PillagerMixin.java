...
public abstract class PillagerMixin {
    ...
    public void performRangedAttack(LivingEntity target, float velocity, CallbackInfo callback) {
        ...
        if (mainHandItem.getItem() instanceof CrossbowItem) {
            CrossbowItem.performShooting(pillager.level(), pillager,
                    InteractionHand.MAIN_HAND, mainHandItem, velocity, 1.0F);
    ...
    @Inject(at = @At("HEAD"),
            method = "shootCrossbowProjectile(Lnet/minecraft/world/entity/LivingEntity;"
                    + "Lnet/minecraft/world/item/ItemStack;"
                    + "Lnet/minecraft/world/entity/projectile/Projectile;F)V")
    private void shootCrossbowProjectile(LivingEntity target, ItemStack crossbow,
            Projectile projectile, float velocity, CallbackInfo info) {
