...
public class RagersBottleItem extends Item {
	...
	public void appendHoverText(ItemStack itemstack, net.minecraft.world.item.Item.TooltipContext level, List<Component> list, TooltipFlag flag) {
		...
		int durationTicks = (ModComponents.has(itemstack) && ModComponents.get(itemstack).contains("BottleEffectDuration")) ? ModComponents.get(itemstack).getInt("BottleEffectDuration") : ItemFeaturesConfig.RAGERS_BOTTLE_EFFECT_DURATION.get();
		int amp = (ModComponents.has(itemstack) && ModComponents.get(itemstack).contains("BottleEffectAmplifier")) ? ModComponents.get(itemstack).getInt("BottleEffectAmplifier") : ItemFeaturesConfig.RAGERS_BOTTLE_EFFECT_AMPLIFIER.get();
		...
		if (EffectsConfig.VIGOROUS_RAGE.get()) {
			...
			Component effectName = MobEffects.MOVEMENT_SPEED.value().getDisplayName().copy().withStyle(ChatFormatting.BLUE);
			list.add(Component.empty().append(effectName).append(Component.literal(" " + romanValue + " (" + timeFormated + ")").withStyle(ChatFormatting.BLUE)));
		}
	}
	...
	private void handleRagersBottleFinish(Player player, ItemStack itemstack, Level world) {
		if (!world.isClientSide()) {
			...
			int duration = (ModComponents.has(itemstack) && ModComponents.get(itemstack).contains("BottleEffectDuration")) ? ModComponents.get(itemstack).getInt("BottleEffectDuration") : ItemFeaturesConfig.RAGERS_BOTTLE_EFFECT_DURATION.get();
			int amplifier = (ModComponents.has(itemstack) && ModComponents.get(itemstack).contains("BottleEffectAmplifier")) ? ModComponents.get(itemstack).getInt("BottleEffectAmplifier") : ItemFeaturesConfig.RAGERS_BOTTLE_EFFECT_AMPLIFIER.get();
			if (EffectsConfig.VIGOROUS_RAGE.get()) {
				player.addEffect(new MobEffectInstance(ModMobEffects.VIGOROUS_RAGE, duration, amplifier));
			} else {
