...
public class ArchersBottleItem extends Item {
	...
	public void appendHoverText(ItemStack itemstack, Level level, List<Component> list, TooltipFlag flag) {
		...
		int durationTicks = (itemstack.hasTag() && itemstack.getTag().contains("BottleEffectDuration")) ? itemstack.getTag().getInt("BottleEffectDuration") : ItemFeaturesConfig.ARCHERS_BOTTLE_EFFECT_DURATION.get();
		int amp = (itemstack.hasTag() && itemstack.getTag().contains("BottleEffectAmplifier")) ? itemstack.getTag().getInt("BottleEffectAmplifier") : ItemFeaturesConfig.ARCHERS_BOTTLE_EFFECT_AMPLIFIER.get();
		...
		if (EffectsConfig.ARCHERS_GRACE.get()) {
			...
			Component effectName = MobEffects.DAMAGE_BOOST.getDisplayName().copy().withStyle(ChatFormatting.BLUE);
			list.add(Component.empty().append(effectName).append(Component.literal(" " + romanValue + " (" + timeFormated + ")").withStyle(ChatFormatting.BLUE)));
		}
	}
	...
	private void handleArchersBottleFinish(Player player, ItemStack itemstack, Level world) {
		if (!world.isClientSide()) {
			...
			int duration = (itemstack.hasTag() && itemstack.getTag().contains("BottleEffectDuration")) ? itemstack.getTag().getInt("BottleEffectDuration") : ItemFeaturesConfig.ARCHERS_BOTTLE_EFFECT_DURATION.get();
			int amplifier = (itemstack.hasTag() && itemstack.getTag().contains("BottleEffectAmplifier")) ? itemstack.getTag().getInt("BottleEffectAmplifier") : ItemFeaturesConfig.ARCHERS_BOTTLE_EFFECT_AMPLIFIER.get();
			if (EffectsConfig.ARCHERS_GRACE.get()) {
				player.addEffect(new MobEffectInstance(ModMobEffects.ARCHERS_GRACE.get(), duration, amplifier));
			} else {
