...
public class CompoundBowItem extends BowItem {
    ...
    public void performMobRangedAttack(Level level, LivingEntity shooter, LivingEntity target, float power) {
        ...
        if (powerLevel > 0) {
            abstractarrow.setBaseDamage(abstractarrow.getBaseDamage() + (double) powerLevel * 0.3D + 0.3D);
        }
        ...
        if (piercingLevel > 0) {
            abstractarrow.setPierceLevel((byte) piercingLevel);
        }
