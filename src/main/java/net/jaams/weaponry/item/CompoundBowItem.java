            ...
            if (!itemstack.isEmpty() || flag) {
                ...
                if (!((double) f < 0.1D)) {
                    ...
                    if (!level.isClientSide) {
                        ...
                        int p = ModEnchantments.level(itemStack, Enchantments.PIERCING);
                        if (p > 0) {
                            ((net.jaams.weaponry.mixins.access.AbstractArrowAccessorMixin) (Object) abstractarrow).invokeSetPierceLevel((byte) p);
                        }
                        if (ModEnchantments.level(itemStack, Enchantments.FLAME) > 0) {
                            abstractarrow.igniteForSeconds(80);
                        }
