...
public class WarriorsBottleItem extends Item {
	...
	public void appendHoverText(ItemStack itemstack, net.minecraft.world.item.Item.TooltipContext level, List<Component> list, TooltipFlag flag) {
		...
		int durationTicks = (ModComponents.has(itemstack) && ModComponents.get(itemstack).contains("BottleEffectDuration")) ? ModComponents.get(itemstack).getInt("BottleEffectDuration") : ItemFeaturesConfig.WARRIORS_BOTTLE_EFFECT_DURATION.get();
		int amp = (ModComponents.has(itemstack) && ModComponents.get(itemstack).contains("BottleEffectAmplifier")) ? ModComponents.get(itemstack).getInt("BottleEffectAmplifier") : ItemFeaturesConfig.WARRIORS_BOTTLE_EFFECT_AMPLIFIER.get();
		...
		if (EffectsConfig.WARRIORS_GRACE.get()) {
			...
			Component effectName = MobEffects.DAMAGE_RESISTANCE.value().getDisplayName().copy().withStyle(ChatFormatting.BLUE);
			list.add(Component.empty().append(effectName).append(Component.literal(" " + romanValue + " (" + timeFormated + ")").withStyle(ChatFormatting.BLUE)));
		}
	}
	...
	private void handleWarriorsBottleFinish(Player player, ItemStack itemstack, Level world) {
		if (!world.isClientSide()) {
			...
			int duration = (ModComponents.has(itemstack) && ModComponents.get(itemstack).contains("BottleEffectDuration")) ? ModComponents.get(itemstack).getInt("BottleEffectDuration") : ItemFeaturesConfig.WARRIORS_BOTTLE_EFFECT_DURATION.get();
			int amplifier = (ModComponents.has(itemstack) && ModComponents.get(itemstack).contains("BottleEffectAmplifier")) ? ModComponents.get(itemstack).getInt("BottleEffectAmplifier") : ItemFeaturesConfig.WARRIORS_BOTTLE_EFFECT_AMPLIFIER.get();
			if (EffectsConfig.WARRIORS_GRACE.get()) {
				player.addEffect(new MobEffectInstance(ModMobEffects.WARRIORS_GRACE, duration, amplifier));
			} else {
