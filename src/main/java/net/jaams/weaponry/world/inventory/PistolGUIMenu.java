...
public class PistolGUIMenu extends BaseGunGUIMenu {

    public PistolGUIMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
        super(ModMenus.PISTOL_GUI.get(), id, inv, extraData);
    }
    ...
    protected ResourceLocation getSlotSound(int slotId) {
        ...
        return ResourceLocation.parse("jaams_weaponry:gun_system_pistol_attachment");
    }

    @Override
    protected void sendSlotPacket(int slotid) {
        JaamsWeaponryMod.PACKET_HANDLER.sendToServer(new PistolGUISlotMessage(slotid, x, y, z, 0, 0));
    }
}
