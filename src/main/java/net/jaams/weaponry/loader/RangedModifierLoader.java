...
public class RangedModifierLoader extends SimpleJsonResourceReloadListener implements NetworkSyncable {
    ...
    public boolean isRangedExplicitlyDisabled(ItemStack stack) {
        ...
        List<RangedItemData> matching = new ArrayList<>();
        for (RangedItemData data : modifiers.values()) {
            if (data == null || data.ranged == null) continue;
        ...
        for (RangedItemData data : matching) {
            return Boolean.FALSE.equals(data.ranged.ranged_enabled);
        }
        return false;
    }
    ...
    private List<RangedItemData> computeRangedModifiersForItem(ResourceLocation itemId) {
        ...
        for (RangedItemData data : modifiers.values()) {
            ...
            if (!Boolean.FALSE.equals(data.ranged.ranged_enabled)) {
                result.add(data);
            }
        }
        result.sort((a, b) -> {
