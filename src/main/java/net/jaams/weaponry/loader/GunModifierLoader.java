...
public class GunModifierLoader extends SimpleJsonResourceReloadListener implements NetworkSyncable {
    ...
    private void rebuild(Map<String, String> srcs) {
        ...
        for (Map.Entry<String, String> entry : srcs.entrySet()) {
            ...
            try {
                ...
                boolean hasGun = data.gun != null && data.gun.gun_type != null && !data.gun.gun_type.isEmpty();
                boolean hasAttachment = data.attachment != null && data.attachment.slot != null
                        && !data.attachment.slot.isEmpty();
                if (!hasGun && !hasAttachment) {
                    LOGGER.warn("Gun modifier file {} has no 'gun' or 'attachment' section defined", fileId);
                    errors++;
                    continue;
                }
                ...
                if (hasAttachment) {
                    data.attachment.slot = data.attachment.slot.toLowerCase(Locale.ROOT);
                    String slot = data.attachment.slot;
                    if (!slot.equals("muzzle") && !slot.equals("magazine") && !slot.equals("any")) {
                        LOGGER.warn("Gun modifier file {}: invalid attachment slot '{}'", fileId,
                                data.attachment.slot);
                        errors++;
                    }
                    if (data.attachment.compatible_guns != null) {
                        for (String pattern : data.attachment.compatible_guns) {
                            if (pattern == null || pattern.isEmpty()) {
                                LOGGER.warn("Gun modifier file {}: empty compatible_guns entry", fileId);
                                errors++;
                            }
                        }
                    }
                    if (data.attachment.modifiers != null) {
                        for (GunItemData.AttachmentModifier mod : data.attachment.modifiers) {
                            if (mod == null || mod.key == null || mod.key.isEmpty()) {
                                LOGGER.warn("Gun modifier file {}: attachment modifier missing key", fileId);
                                errors++;
                            }
                        }
                    }
    ...
    private List<GunItemData> computeGunModifiersForItem(ResourceLocation itemId) {
        ...
        for (GunItemData data : guns.values()) {
            ...
            boolean hasAttachment = data.attachment != null && data.attachment.slot != null
                    && !data.attachment.slot.isEmpty();
            if (hasGun && data.gun.gun_enabled) {
                result.add(data);
    ...
    public boolean isGunExplicitlyDisabled(ItemStack stack) {
        if (stack == null || stack.isEmpty()) {
            return false;
        }
        CompoundTag tag = ModComponents.get(stack);
        if (tag != null && tag.contains("GunType", CompoundTag.TAG_STRING)) {
            String customType = tag.getString("GunType").toUpperCase(Locale.ROOT).trim();
            switch (customType) {
                case "PISTOL", "SCATTERGUN", "SHOTGUN", "GUN", "REVOLVER", "PEPPERBOX" -> {
                    return false;
                }
            }
        }
        ...
        List<GunItemData> matching = new ArrayList<>();
        for (GunItemData data : guns.values()) {
            if (data == null || data.gun == null) {
                continue;
            }
            boolean hasGun = data.gun.gun_type != null && !data.gun.gun_type.isEmpty();
            if (!hasGun) {
                continue;
            }
            if (!matchesTarget(data.target, itemId)) {
            ...
            if (!evaluateConditions(data, stack)) {
                continue;
            }
            matching.add(data);
        }
        ...
        for (GunItemData data : matching) {
            return !data.gun.gun_enabled;
        }
        return false;
    }
