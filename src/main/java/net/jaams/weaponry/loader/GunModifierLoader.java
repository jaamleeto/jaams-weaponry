...
public class GunModifierLoader extends SimpleJsonResourceReloadListener implements NetworkSyncable {
    ...
    private void rebuild(Map<String, String> srcs) {
        ...
        for (Map.Entry<String, String> entry : srcs.entrySet()) {
            ...
            try {
                ...
                if (hasAttachment) {
                    String slot = data.attachment.slot.toLowerCase(Locale.ROOT);
    ...
    public boolean isGunExplicitlyDisabled(ItemStack stack) {
        ...
        if (tag != null && tag.contains("GunType", CompoundTag.TAG_STRING)) {
            String customType = tag.getString("GunType").toUpperCase().trim();
            switch (customType) {
                case "PISTOL":
                case "SCATTERGUN":
                case "SHOTGUN":
        ...
        for (GunItemData data : guns.values()) {
            if (data == null || data.gun == null) continue;
            boolean hasGun = data.gun.gun_type != null && !data.gun.gun_type.isEmpty();
            if (!hasGun) continue;
            if (!matchesTarget(data.target, itemId)) continue;
            if (!evaluateConditions(data, stack)) continue;
            matching.add(data);
        }
        matching.sort((a, b) -> {
            int byPriority = Integer.compare(b.priority, a.priority);
