...
public class ProjectileSpecialHandler {
    ...
    private void handleSpawnEggImpact(Vec3 impactPos) {
        ...
        EntityType<?> type = spawnEgg.getType(proj);
    ...
    private void handleEnderPearlImpact() {
        ...
        if (owner instanceof ServerPlayer serverPlayer) {
            if (serverPlayer.connection.isAcceptingMessages() && serverPlayer.level() == level
                ...
                net.neoforged.neoforge.event.entity.EntityTeleportEvent.EnderPearl event = net.neoforged.neoforge.event.EventHooks
    ...
    private void handleArmorEquipImpact(Entity target, ItemStack stack) {
        ...
        if (!currentInSlot.isEmpty()
                && !(currentInSlot.getItem() instanceof ArmorItem currentArmor && item instanceof ArmorItem newArmor
                        && newArmor.getMaterial().value().getDefense(((ArmorItem) item).getType()) > currentArmor
                                .getMaterial().value().getDefense(currentArmor.getType()))) {
            return;
        }
        livingTarget.setItemSlot(slot, stack.copy());
        livingTarget.level().playSound(null, livingTarget.blockPosition(), SoundEvents.ARMOR_EQUIP_GENERIC.value(),
                SoundSource.PLAYERS, 1.0F, 1.0F);
        projectile.discard();
    }
    ...
    private void handleFireImpact(Entity target, ItemStack stack) {
        ...
        livingTarget.igniteForSeconds(6);
    ...
    private void handleExperienceBottleImpact() {
        if (projectile.level() instanceof ServerLevel serverLevel) {
            serverLevel.levelEvent(2002, projectile.blockPosition(), PotionContents.getColor(Potions.WATER));
