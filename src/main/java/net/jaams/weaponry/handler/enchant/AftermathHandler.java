...
public class AftermathHandler {
    ...
    public static void onAftermathLivingEntityUseItemStop(LivingEntityUseItemEvent.Stop event) {
        ...
        if (entity instanceof Player player) {
            int aftermathLevel = EnchantmentHelper.getTagEnchantmentLevel(ModEnchantments.AFTERMATH.get(), item);
            if (aftermathLevel > 0) {
    ...
    @SubscribeEvent
    public static void onAftermathPlayerTick(TickEvent.PlayerTickEvent event) {
    ...
    @SubscribeEvent
    public static void onAftermathServerTick(TickEvent.ServerTickEvent event) {
    ...
    @SubscribeEvent
    public static void onAftermathLivingHurt(LivingHurtEvent event) {
    ...
    private static void aftermathProcessDelayedDamage(LivingEntity target, Entity sourceEntity, float extraDamage,
        ...
        if (EnchantmentsConfig.AFTERMATH_PROTECTION_REDUCES_DAMAGE.get()) {
            int epf = EnchantmentHelper.getDamageProtection(target.getArmorSlots(), damageSource);
            if (epf > 0) {
                finalCalculatedDamage = net.minecraft.world.damagesource.CombatRules
                        .getDamageAfterMagicAbsorb(finalCalculatedDamage, (float) epf);
            }
        }
    ...
    private static void aftermathHandleThornsReflection(LivingEntity target, Entity sourceEntity, float damageBasis,
        ...
        int thornsLevel = EnchantmentHelper.getEnchantmentLevel(Enchantments.THORNS, target);
        if (thornsLevel <= 0) {
            return;
        }
