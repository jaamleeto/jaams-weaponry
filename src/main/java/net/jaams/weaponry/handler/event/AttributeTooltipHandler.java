...
public class AttributeTooltipHandler {
    ...
    public static void onAttributeTooltip(ItemTooltipEvent e) {
        ...
        attributeOrder.put("attribute.name.player.entity_interaction_range", 4);
        ...
        // Attributes that use PERCENTAGE display type - ADD_VALUE shows as x100 with %
        Set<String> percentageAttributes = new HashSet<>
                (Arrays.asList("attribute.name.generic.movement_speed",
        ...
        for (Iterator<Component> it = lines.iterator(); it.hasNext();) {
            ...
            if (content instanceof TranslatableContents ttc) {
                if (ttc.getKey().startsWith("item.modifiers")) {
                    ...
                    // For percentage attributes, vanilla shows ADD_VALUE * 100, so divide by 100
                    if (percentageAttributes.contains(attrName)) {
                        value = value / 100.0;
                    }
