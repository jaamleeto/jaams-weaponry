...
public class BonusDamageHandler {
    ...
    private static boolean matchesArthropodTarget(LivingEntity target) {
        if (target.getType().is(ModTags.ARTHROPOD_GRUDGE_TARGETS))
            return true;
        return target.getType().is(EntityTypeTags.ARTHROPOD);
    }
    ...
    private static boolean matchesBoneTarget(LivingEntity target) {
        if (target.getType().is(ModTags.BONE_GRUDGE_TARGETS))
            return true;
    ...
    private static boolean matchesRottenTarget(LivingEntity target) {
        if (target.getType().is(ModTags.ROTTEN_GRUDGE_TARGETS))
            return true;
    ...
    private static boolean matchesSnoutTarget(LivingEntity target) {
        if (target.getType().is(ModTags.SNOUT_GRUDGE_TARGETS))
            return true;
        if (target instanceof AbstractPiglin || target instanceof Hoglin)
            return true;
        String entityId = BuiltInRegistries.ENTITY_TYPE.getKey(target.getType()).toString().toLowerCase();
    ...
    private static boolean matchesTraitorTarget(LivingEntity target) {
        if (target.getType().is(ModTags.TRAITOR_GRUDGE_TARGETS))
            return true;
        return target instanceof AbstractIllager;
    }
    ...
    private static boolean matchesUndeadTarget(LivingEntity target) {
        if (target.getType().is(ModTags.UNDEAD_GRUDGE_TARGETS))
            return true;
