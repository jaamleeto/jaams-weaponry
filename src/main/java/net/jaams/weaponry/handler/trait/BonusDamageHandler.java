...
public class BonusDamageHandler {
    ...
    private static boolean matchesAquaticTarget(LivingEntity target) {
        if (target.getType().is(ModTags.AQUATIC_GRUDGE_TARGETS)
                || target.getMobType() == MobType.WATER
    ...
    public static void handleArthropodGrudge(LivingEntity target, LivingEntity attacker, ItemStack stack) {
        ...
        if (!TraitsConfig.ARTHROPOD_GRUDGE.get())
            return;
        if (!matchesArthropodTarget(target))
            return;
    ...
    private static boolean matchesArthropodTarget(LivingEntity target) {
        return target.getType().is(ModTags.ARTHROPOD_GRUDGE_TARGETS)
                || target.getMobType() == MobType.ARTHROPOD;
    }
    ...
    private static boolean matchesBoneTarget(LivingEntity target) {
        if (target.getType().is(ModTags.BONE_GRUDGE_TARGETS) || target instanceof AbstractSkeleton)
            return true;
        String entityId = BuiltInRegistries.ENTITY_TYPE.getKey(target.getType()).toString().toLowerCase();
        return entityId.contains("skeleton");
    }
    ...
    private static boolean matchesRottenTarget(LivingEntity target) {
        if (target.getType().is(ModTags.ROTTEN_GRUDGE_TARGETS) || target instanceof Zombie)
            return true;
        String entityId = BuiltInRegistries.ENTITY_TYPE.getKey(target.getType()).toString().toLowerCase();
    ...
    private static boolean matchesSnoutTarget(LivingEntity target) {
        if (target.getType().is(ModTags.SNOUT_GRUDGE_TARGETS) || target instanceof AbstractPiglin || target instanceof Hoglin)
            return true;
        String entityId = BuiltInRegistries.ENTITY_TYPE.getKey(target.getType()).toString().toLowerCase();
        return entityId.contains("piglin") || entityId.contains("hoglin");
    }
    ...
    public static void handleTraitorGrudge(LivingEntity target, LivingEntity attacker, ItemStack stack) {
        ...
        if (!matchesTraitorTarget(target))
            return;
    ...
    private static boolean matchesTraitorTarget(LivingEntity target) {
        return target.getType().is(ModTags.TRAITOR_GRUDGE_TARGETS)
    ...
    private static boolean matchesUndeadTarget(LivingEntity target) {
        return target.getType().is(ModTags.UNDEAD_GRUDGE_TARGETS)
    ...
    private static float calculateBackstabBaseDamage(ItemStack stack, LivingEntity attacker) {
        ...
        float baseDamage = modifier != null ? (float) modifier.getAmount() : 1.0f;
    ...
    private static void applyBackstabFinalEffects(LivingEntity attacker, LivingEntity target, ItemStack stack,
        ...
        stack.hurtAndBreak((int) durabilityPenalty, attacker, e -> e.broadcastBreakEvent(e.getUsedItemHand()));
    }
