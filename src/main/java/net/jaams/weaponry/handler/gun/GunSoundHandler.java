...
public class GunSoundHandler {
    ...
    public static void handleWeaponFire(
        ...
        float volume = getFinalFloat(itemstack, "GunSoundVolume", soundData != null ? soundData.sound_volume : -1.0f, attachmentVolume, 1.0f);
        float pitch = getFinalFloat(itemstack, "GunSoundPitch", soundData != null ? soundData.sound_pitch : -1.0f, attachmentPitch, 1.0f);
        double attachmentBulletDropChance = GunAttachmentHelper.getAttachmentBulletDropChance(itemstack);
        double bulletDropChance = getFinalDouble(itemstack, "GunBulletDropChance", soundData != null ? soundData.bullet_drop_chance : -1.0, attachmentBulletDropChance, 0.7);
        int attachmentAfterShotDelay = GunAttachmentHelper.getAttachmentAfterShotDelay(itemstack);
        int afterShotDelay = getFinalInt(itemstack, "GunAfterShotDelay", soundData != null ? soundData.after_shot_delay : -1, attachmentAfterShotDelay, 10);
        int attachmentEmptyCooldown = GunAttachmentHelper.getAttachmentEmptyCooldown(itemstack);
        int emptyCooldown = getFinalInt(itemstack, "GunEmptyCooldown", soundData != null ? soundData.empty_cooldown : -1, attachmentEmptyCooldown, 20);
        ...
        if (magazineAttachment != null) {
            int magazineSlot = GunAttachmentHelper.getMagazineSlot();
