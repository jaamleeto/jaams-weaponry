...
public class VigorousRageHandler {
    ...
    public static void onVigorousRage(LivingDeathEvent event) {
        ...
        if (!attacker.hasEffect(ModMobEffects.VIGOROUS_RAGE.get())) {
            return;
        }
        LivingEntity target = event.getEntity();
        if (isValidTarget(target)) {
            MobEffectInstance effectInstance = attacker.getEffect(ModMobEffects.VIGOROUS_RAGE.get());
            ...
            MobEffectInstance newEffect = new MobEffectInstance(ModMobEffects.VIGOROUS_RAGE.get(), newDuration,
                    newLevel - 1);
