...
public class RestrictedEffectHandler {
    ...
    public static void onRestrictedItemUse(PlayerInteractEvent.RightClickItem event) {
        ...
        if (restrictKnockedOutItemUse() && player.hasEffect(ModMobEffects.KNOCKED_OUT.get())) {
            cancel = true;
        }
        if (restrictIncapableItemUse() && player.hasEffect(ModMobEffects.INCAPABLE.get())) {
            cancel = true;
        }
        if (cancel) {
    ...
    public static void onKnockedOutRightClickItem(PlayerInteractEvent.RightClickItem event) {
        if (restrictKnockedOutItemUse() && event.getEntity().hasEffect(ModMobEffects.KNOCKED_OUT.get())) {
            event.setCanceled(true);
        }
    }
    ...
    public static void onKnockedOutJump(LivingEvent.LivingJumpEvent event) {
        ...
        if (entity.hasEffect(ModMobEffects.KNOCKED_OUT.get())) {
    ...
    @SubscribeEvent
    public static void onKnockedOutAttack(LivingHurtEvent event) {
        if (!isKnockedOutEnabled()) return;
        if (!EffectsConfig.KNOCKED_OUT_RESTRICT_ATTACKING.get()) return;
        DamageSource source = event.getSource();
        if (source.getEntity() instanceof LivingEntity attacker && attacker.hasEffect(ModMobEffects.KNOCKED_OUT.get())) {
            event.setCanceled(true);
        }
    }
}
