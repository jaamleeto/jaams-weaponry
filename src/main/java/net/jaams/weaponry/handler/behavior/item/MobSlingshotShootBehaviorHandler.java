...
public class MobSlingshotShootBehaviorHandler {
    ...
    private static boolean isAmmoItem(ItemStack stack, ItemStack slingshotStack) {
        ...
        if (slingshotStack.hasTag() && slingshotStack.getTag().contains("SlingshotAmmoItems", 9)) {
            var tag = slingshotStack.getTag().getList("SlingshotAmmoItems", 8);
            if (!tag.isEmpty()) {
                ResourceLocation ammoId = ForgeRegistries.ITEMS.getKey(stack.getItem());
