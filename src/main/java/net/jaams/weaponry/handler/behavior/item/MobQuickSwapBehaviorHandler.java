    ...
    private static Item resolveBySuffix(ItemStack stack) {
        ...
        for (String suffix : SUFFIXES) {
            if (path.endsWith(suffix)) {
                ...
                Item item = BuiltInRegistries.ITEM.get(ResourceLocation.fromNamespaceAndPath(ns, base));
                if (isValidTarget(item)) return item;
            }
        }
        for (String suffix : SUFFIXES) {
            Item item = BuiltInRegistries.ITEM.get(ResourceLocation.fromNamespaceAndPath(ns, path + suffix));
            if (isValidTarget(item)) return item;
        }
        return null;
    }
